...
program PascalS_mod(input { + [sam] } , output, srcfil { [sam] } ); (* 1.6.75 *)
...
begin
  ...
  msg[6] := 'code      ';
  msg[7] := 'strings   ';
  writeln(' compiler table for ', string(msg[n]), ' is too small');
...
begin
  if CharInSet(c, ['A'..'Z']) then
    lowerchar := Chr(Ord(c) + Ord('a') - Ord('A'))
  else
    lowerchar := c;
end (* lowerchar *);
...
begin (* insymbol *)
  ...
  while True do
  begin
    while CharInSet(ch, [#$00..#$20, #$7F]) do
      nextch;
    if CharInSet(ch, ['_', 'A'..'Z', 'a'..'z']) then
    begin (* word *)
      k := 0;
      id := '          ';
      repeat
        ...
        nextch
      until not CharInSet(ch, ['_', 'A'..'Z', 'a'..'z', '0'..'9']);
      i := 1;
      j := nkw; (* binary search *)
      repeat
        k := (i + j) div 2;
    ...
    begin (* number *)
      ...
      begin
        ...
        begin
          ...
          if lowerchar(ch) = 'e' then
            readscale;
          if e <> 0 then
            adjustscale
        end
      end
      else if lowerchar(ch) = 'e' then
      begin
        sy := realcon;
        rnum := inum;
        e := 0;
...
begin
  ...
  for i := btab[1].last + 1 to t do
    with tab[i] do
      writeln(i, ' ', string(name), link:5, ord(obj):5, ord(typ):5, ref:5,
...
var
  ...
  function lower(s: alfa): alfa;
  var
    i: integer;
  begin
    for i:=1 to 10 do
      lower[i] := lowerchar(s[i]);
  end (* lower *);
...
begin (* interpret *)
  ...
  begin
    ...
    repeat
      ...
      h2 := s[h1 + 4].i;
      if h1 <> 0 then
        writeln(' ', string(tab[h2].name), ' called at',
          s[h1 + 1].i:5);
